...
async def load_env_variables(settings: Settings) -> None:
    ...
    if not settings.groq_api_key:
        settings.groq_api_key = getpass.getpass("GROQ_API_KEY: ")
...
class Assistant(Agent):
    def __init__(self, settings: Settings) -> None:
        ...
        super().__init__(
            instructions="""You are a voice assistant created by Extrawest Company. You communicate with users through voice interface.
...
Special Functions:
- For weather queries: Always use the get_weather function with appropriate latitude/longitude
- For information queries: Use the search_web function to find accurate information

About Me:
- I am a demo assistant showcasing LiveKit's agents framework capabilities
- I was created by Extrawest Company to demonstrate voice interaction technology

Remember: Never make up weather information - always use the get_weather function for weather-related questions."""
        )
    ...
    async def get_weather(
        ...
        url = f"https://api.openweathermap.org/data/2.5/weather?lat={latitude}&lon={longitude}&lang=en&units=metric&appid={self.settings.weather_api_key}"
        weather_data = {}
...
async def entrypoint(ctx: agents.JobContext):
    ...
    session = AgentSession(
        ...
        llm=OpenAILLM(
            ...
            temperature=0.1,
            timeout=30.0  # Increasing timeout to avoid API timeouts
        ),
        tts=OpenAITTS(
